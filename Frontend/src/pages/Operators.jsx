...
const Operators = () => {
    ...
    const renderOperatorRow = (operator) => (
        <tr key={operator._id} className="transition-colors hover:bg-slate-800/60">
            <td className="px-6 py-4"> {operator.avatar ? <img src={operator.avatar} alt={operator.name} className="h-10 w-10 rounded-full object-cover"/> : <div className="h-10 w-10 rounded-full bg-slate-700 flex items-center justify-center"><User className="h-5 w-5 text-slate-400" /></div>} </td>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300"><span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-medium ${ operator.role === 'Admin' ? 'bg-amber-500/10 text-amber-400' : 'bg-sky-500/10 text-sky-400' }`}> {operator.role === 'Admin' && <Shield className="h-3 w-3" />} {operator.role}</span></td>
        </tr>
    );
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6"><h1 className="text-3xl font-bold text-white">Manage Operators</h1><Button onClick={handleOpenModal} className="gap-2"><PlusCircle className="h-5 w-5"/> Add Operator</Button></div>
            {error && <p className="text-red-500 bg-red-500/10 p-3 rounded-md mb-4">{error}</p>}
            <Table headers={["", "Name", "Email", "Contact", "Role"]} data={operators} renderRow={renderOperatorRow} />
