...
const Operators = () => {
    ...
    const processedOperators = useMemo(() => {
        ...
        sortableItems.sort((a,b) => {
            let x = a[sortConfig.key]?.toLowerCase() || '';
            let y = b[sortConfig.key]?.toLowerCase() || '';
    ...
    const renderOperatorRow = (op, index) => (
        <tr key={op._id} className="transition-colors hover:bg-background-offset" style={{ animation: 'tableRowFadeIn 0.5s ease-out forwards', animationDelay: `${index * 0.03}s`, opacity: 0 }}>
            <td className="px-6 py-4">{op.avatar ? <img src={op.avatar} alt={op.name} className="h-10 w-10 rounded-full object-cover"/> : <div className="h-10 w-10 rounded-full bg-background-offset/50 flex items-center justify-center"><User className="h-5 w-5 text-text-secondary"/></div>}</td>
            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-text-primary">{op.name}</td>
            <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{op.email}</td>
            <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{op.contactNumber}</td>
            <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary"><span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-medium ${op.role === 'Admin' ? 'bg-amber-500/10 text-amber-500' : 'bg-sky-500/10 text-sky-500'}`}>{op.role === 'Admin' && <Shield className="h-3 w-3"/>}{op.role}</span></td>
        </tr>
    );
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                <h1 className="text-3xl font-bold text-glow">Manage Operators</h1>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Register New Operator">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div>
                        ...
                        <Input type="file" {...register('avatar')} className="file:bg-background-offset file:text-text-primary file:border-0 file:rounded file:py-1.5 file:px-3 file:mr-3 hover:file:bg-border"/>
                    ...
                    <div className="flex justify-end gap-2 pt-4">
                        ...
                        <Button type="submit" loading={isSubmitting}>Register Operator</Button>
                    </div>
                </form>
            </Modal>
        </div>
