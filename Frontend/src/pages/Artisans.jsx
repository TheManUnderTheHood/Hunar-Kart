...
const Artisans = () => {
    ...
    const fetchArtisans = async () => { try { setLoading(true); const response = await apiClient.get('/artisans'); setArtisans(response.data.data.artisans || []); } catch (err) { toast.error('Failed to fetch artisans.'); } finally { setLoading(false); } };
    ...
    const onSubmit = async (data) => { await toast.promise(currentArtisan ? apiClient.patch(`/artisans/${currentArtisan._id}`, data) : apiClient.post('/artisans', data), { loading: currentArtisan ? 'Updating...' : 'Creating...', success: (res) => { fetchArtisans(); handleCloseModal(); return res.data.message || 'Saved!'; }, error: (err) => err.response?.data?.message || 'Error.', }); };
    const handleDelete = async (artisanId) => { if (window.confirm('Delete? This is permanent.')) { await toast.promise(apiClient.delete(`/artisans/${artisanId}`), { loading: 'Deleting...', success: 'Deleted!', error: 'Failed.' }).then(() => { setArtisans(prev => prev.filter(a => a._id !== artisanId)); }); } };
    ...
    return (
        <div>
            ...
            <Table
                ...
                renderRow={(artisan, index) => (
                    <tr key={artisan._id} className="transition-colors hover:bg-slate-800/60" style={{ animation: 'tableRowFadeIn 0.5s ease-out forwards', animationDelay: `${index * 0.03}s`, opacity: 0 }}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium"><div className='flex gap-1'>
                            <Link to={`/artisans/${artisan._id}`}><Button variant="ghost" className="p-2 h-auto"><Eye className="h-4 w-4" /></Button></Link>
