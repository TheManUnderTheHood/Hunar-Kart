...
const HandcraftedItems = () => {
    ...
    const fetchData = async () => { try { setLoading(true); const [i, a] = await Promise.all([apiClient.get('/handcrafteditem'), apiClient.get('/artisans')]); setItems(i.data.data.items || []); setArtisans(a.data.data.artisans || []); } catch(e){ toast.error('Failed to fetch data.'); } finally { setLoading(false); } };
    useEffect(() => { fetchData() }, []);
    ...
    const onSubmit = async (data) => {
        const promise = currentItem ? apiClient.patch(`/handcrafteditem/${currentItem._id}`, data) : apiClient.post('/handcrafteditem', data);
        await toast.promise(promise, { loading: 'Saving item...', success: 'Item saved successfully!', error: err => err.response?.data?.message || 'Failed to save item.' })
            .then(() => { fetchData(); handleCloseModal(); });
    };
    const handleDelete = async (id) => { if (window.confirm('Are you sure?')) { const promise = apiClient.delete(`/handcrafteditem/${id}`); await toast.promise(promise, { loading: 'Deleting...', success: 'Deleted!', error: 'Failed.' }).then(() => setItems(p => p.filter(i => i._id !== id))) } };
    const processedItems = useMemo(() => { let s=[...items];s.sort((a,b)=>{let av=a[sortConfig.key],bv=b[sortConfig.key];if(sortConfig.key==='artisanID'){av=a.artisanID?.name||'';bv=b.artisanID?.name||''}if(av<bv)return sortConfig.direction==='ascending'?-1:1;if(av>bv)return sortConfig.direction==='ascending'?1:-1;return 0});return s.filter(i=>i.name.toLowerCase().includes(searchTerm.toLowerCase())) }, [items,searchTerm,sortConfig]);
