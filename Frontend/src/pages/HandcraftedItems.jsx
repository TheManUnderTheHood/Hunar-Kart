...
const HandcraftedItems = () => {
    ...
    const handleDelete = async (itemId) => {
        if (window.confirm('Are you sure?')) {
            try { await apiClient.delete(`/handcrafteditem/${itemId}`); fetchData(); }
            catch (err) { setError(err.response?.data?.message || 'Failed to delete item.'); }
        }
    };
    ...
    const renderRow = (item) => (
        <tr key={item._id} className="transition-colors hover:bg-slate-800/60">
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium"><div className='flex gap-2'><Button variant="ghost" onClick={() => handleOpenModal(item)} className="p-2 h-auto"><Edit className="h-4 w-4"/></Button><Button variant="ghost" onClick={() => handleDelete(item._id)} className="p-2 h-auto text-red-500 hover:bg-red-500/10 hover:text-red-400"><Trash2 className="h-4 w-4"/></Button></div></td>
        </tr>
    );
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6"><h1 className="text-3xl font-bold text-white">Handcrafted Items</h1><Button onClick={() => handleOpenModal()} className="gap-2"><PlusCircle className="h-5 w-5"/> Add Item</Button></div>
