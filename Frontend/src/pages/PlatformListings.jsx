...
// Re-using helper components from HandcraftedItems.jsx
...
const PlatformListings = () => {
    ...
    const handleDelete = async (listingId) => {
        if (window.confirm('Are you sure?')) {
            try {
                await apiClient.delete(`/platformlisting/${listingId}`);
                fetchData();
            } catch (err) {
    ...
    const tableHeaders = ["Item", "Platform", "Status", "URL", "Actions"];
    ...
    const renderRow = (listing) => (
        <tr key={listing._id}>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300"><a href={listing.listingURL} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{listing.listingURL}</a></td>
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <Button onClick={() => handleOpenModal()} className="gap-2"><PlusCircle className="h-5 w-5"/> Add Listing</Button>
            </div>
            {error && <p className="text-red-500 bg-red-500/10 p-3 rounded-md mb-4">{error}</p>}
            <Table headers={tableHeaders} data={listings} renderRow={renderRow} />
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={currentListing ? 'Edit Listing' : 'Add New Listing'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-2 pt-4">
                        ...
                        <Button type="submit">{currentListing ? 'Save Changes' : 'Create Listing'}</Button>
                    </div>
                </form>
            </Modal>
        </div>
