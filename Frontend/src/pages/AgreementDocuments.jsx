...
const AgreementDocuments = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [docsRes, artisansRes] = await Promise.all([ apiClient.get('/agreementdocument'), apiClient.get('/artisans') ]);
    ...
    const handleDelete = async (docId) => {
        if (window.confirm('Are you sure?')) {
            try { await apiClient.delete(`/agreementdocument/${docId}`); fetchData(); } 
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6"><h1 className="text-3xl font-bold text-white">Agreement Documents</h1><Button onClick={handleOpenModal} className="gap-2"><PlusCircle className="h-5 w-5"/> Add Document</Button></div>
            {error && <p className="text-red-500 bg-red-500/10 p-3 rounded-md mb-4">{error}</p>}
            <Table headers={["Artisan", "Date Signed", "Valid Until", "Actions"]} data={documents} renderRow={renderRow} />
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Upload New Agreement">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-2 pt-4">
                        ...
                        <Button type="submit" loading={isSubmitting}>Upload Document</Button>
                    </div>
                </form>
            </Modal>
        </div>
