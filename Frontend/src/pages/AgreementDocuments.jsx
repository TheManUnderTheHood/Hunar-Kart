...
const AgreementDocuments = () => {
    ...
    useEffect(() => { const fetchData = async () => { try { setLoading(true); const [d, a] = await Promise.all([apiClient.get('/agreementdocument'), apiClient.get('/artisans')]); setDocuments(d.data.data.documents || []); setArtisans(a.data.data.artisans || []); } catch(e) { toast.error('Failed to fetch agreement data.'); } finally { setLoading(false); } }; fetchData(); }, []);
    ...
    const handleDelete = async (docId) => { if(window.confirm('Delete?')){await toast.promise(apiClient.delete(`/agreementdocument/${docId}`),{loading:'Deleting...',success:'Deleted!',error:'Failed.'}).then(()=>setDocuments(p=>p.filter(d=>d._id!==docId)))}};
    const processedDocuments = useMemo(() => { let d=[...documents];d.sort((a,b)=>{let x=a[sortConfig.key],y=b[sortConfig.key];if(sortConfig.key==='artisanID'){x=a.artisanID?.name||'';y=b.artisanID?.name||''}if(x<y)return sortConfig.direction==='ascending'?-1:1;if(x>y)return sortConfig.direction==='ascending'?1:-1;return 0;});return d.filter(doc=>doc.artisanID?.name.toLowerCase().includes(searchTerm.toLowerCase()));},[documents,searchTerm,sortConfig]);
    ...
    const renderRow = (doc, index) => (
        <tr key={doc._id} className="transition-colors hover:bg-background-offset" style={{ animation: 'tableRowFadeIn 0.5s ease-out forwards', animationDelay: `${index * 0.03}s`, opacity: 0 }}>
            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-text-primary">{doc.artisanID?.name||'N/A'}</td>
            <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{new Date(doc.dateSigned).toLocaleDateString()}</td>
            <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{doc.validUntil?new Date(doc.validUntil).toLocaleDateString():'N/A'}</td>
    ...
    return (
        <div>
            ...
            <Table headers={tableHeaders.map(h => (<div onClick={() => h.sortable && requestSort(h.key)} className={`flex items-center ${h.sortable ? 'cursor-pointer' : ''}`}>{h.name}{getSortIcon(h.key)}</div>))} data={processedDocuments} renderRow={renderRow} />
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Upload New Agreement">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div><label htmlFor="agreementFile" className="block text-sm font-medium text-text-secondary mb-1">Agreement File (PDF, etc.)</label><Input id="agreementFile" type="file" {...register('agreementFile')} className="file:bg-background-offset file:text-text-primary file:border-0 file:rounded file:py-1.5 file:px-3 file:mr-3 hover:file:bg-border"/><FormError message={errors.agreementFile?.message}/></div>
